...
function initializeBoard() {
    ...
    for (let row = 0; row < boardHeight; row++) {
        for (let col = 0; col < boardWidth; col++) {
            ...
            cell.className = "cell";
            cell.style.backgroundColor = blockColors[board[row][col]]; // Set background color
            cell.textContent = board[row][col]; // Set block value as text
            cell.setAttribute("row", row); // Set row attribute
            cell.setAttribute("col", col); // Set col attribute
    ...
    for (let row = 0; row < boardHeight; row++) {
        for (let col = 0; col < boardWidth; col++) {
            ...
            if (board[row][col] > 0) {
                ...
                cell.textContent = ""; // Clear text if the cell is empty
            }
        }
    }
}
        ...
        for (let col = 0; col < boardWidth; col++) {
            if (board[row][col] !== 0) {
                ...
                if (col < boardWidth - 1 && board[row][col] === board[row][col + 1]) {
                    ...
                    console.log("before anima")
                    // スライドアニメーションを適用
                    const cell = document.querySelector(`.cell[row="${row}"][col="${col}"]`);
                    console.log(cell,"cell")
                    if (cell) {
                        console.log("start anima")
                        cell.style.animationName = "slide"; // スライドアニメーションを開始
